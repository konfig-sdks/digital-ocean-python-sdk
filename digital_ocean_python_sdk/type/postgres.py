...
from digital_ocean_python_sdk.type.pgbouncer import Pgbouncer
from digital_ocean_python_sdk.type.timescaledb import Timescaledb

RequiredPostgres = TypedDict("RequiredPostgres", {
    })

OptionalPostgres = TypedDict("OptionalPostgres", {
    ...
    "pg_partman_bgw.role": str,

    # Sets the time interval to run pg_partman's scheduled tasks.
    "pg_partman_bgw.interval": int,

    # Controls which statements are counted. Specify 'top' to track top-level statements (those issued directly by clients), 'all' to also track nested statements (such as statements invoked within functions), or 'none' to disable statement statistics collection. The default value is top.
    "pg_stat_statements.track": str,
...
class Postgres(RequiredPostgres, OptionalPostgres):
    pass
